...
测试包含功能测试与基准测试（`test_bench_*.py`），基准在确定性的关键词回退模式下运行（禁用 Embedding，无需下载模型），含生成式大规模语料与多轮中位数统计：

| 基准 | 规模 | 指标（断言下界 / 实测值） |
|------|------|--------------------------|
| `test_bench_search.py` | 16 文件 + 12 主题查询；250 文件生成语料 + 42 单目标查询；30,000 chunk 延迟库 | Top-5 召回 ≥ 90%（实测 100%，MRR@10=1.000）；30k chunk 中位延迟 < 200ms（实测 ~50ms） |
| `test_bench_classify.py` | 24 文件对抗语料 + 204 文件现实比例语料 | 多策略比单一后缀规则提升 ≥ 15pp（实测现实语料 47.1% → 100%） |
